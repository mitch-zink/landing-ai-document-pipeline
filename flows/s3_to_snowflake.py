...
DEPLOY_MODE = True  # Set to False to run directly, True to deploy
...
def extract_document_content(file_content: bytes, file_key: str) -> Dict[str, Any]:
    ...
    get_run_logger().info(
        f"Making Landing AI API call for {file_key} using official library"
    )
    ...
    try:
        ...
        if results and len(results) > 0:
                ...
                for chunk in result.chunks:
                    chunk_dict = {
                        ...
                        "chunk_type": (
                            ...
                            and hasattr(chunk.chunk_type, "value")
                            else "text"
                        ),
                        "chunk_id": (
                    ...
                    if hasattr(chunk, "grounding") and chunk.grounding:
                        ...
                        for grounding in chunk.grounding:
                            grounding_dict = {
                                "page": (
                                    grounding.page if hasattr(grounding, "page") else 0
                                )
                            }
                            if hasattr(grounding, "box"):
                                grounding_dict["box"] = {
                                    "l": (
                                        grounding.box.l
                                        if hasattr(grounding.box, "l")
                                        else 0
                                    ),
                                    "t": (
                                        grounding.box.t
                                        if hasattr(grounding.box, "t")
                                        else 0
                                    ),
                                    "r": (
                                        grounding.box.r
                                        if hasattr(grounding.box, "r")
                                        else 0
                                    ),
                                    "b": (
                                        grounding.box.b
                                        if hasattr(grounding.box, "b")
                                        else 0
                                    ),
                                }
            ...
            get_run_logger().warning(
                "Landing AI returned empty results, using mock response"
            )
...
def setup_snowflake_infrastructure() -> None:
    ...
    with get_snowflake_connection(warehouse="PREFECT_WH") as conn:
        with conn.cursor() as cur:
            ...
            _exec_sql(
                cur,
                "CREATE WAREHOUSE IF NOT EXISTS PREFECT_WH WITH WAREHOUSE_SIZE = 'X-SMALL'",
                "Create warehouse",
            )
            ...
            _exec_sql(
                cur,
                "CREATE SCHEMA IF NOT EXISTS AGENTIC_DOC_EXTRACTION",
                "Create schema",
            )
            ...
            try:
                ...
                get_run_logger().info(
                    f"Context - DB: {context[0]}, Schema: {context[1]}, WH: {context[2]}"
                )
            except Exception as e:
                get_run_logger().warning(f"Could not fetch context: {e}")
            ...
            try:
                ...
                _exec_sql(
                    cur,
                    table_sql.replace("ai.AGENTIC_DOC_EXTRACTION.", ""),
                    "Create simple table",
                )
...
def load_to_snowflake(file_key: str, extracted_content: Dict[str, Any]) -> None:
    ...
    with get_snowflake_connection(
        warehouse="PREFECT_WH", database="ai", schema="AGENTIC_DOC_EXTRACTION"
    ) as conn:
        with conn.cursor() as cur:
            _exec_sql(cur, "USE DATABASE ai", "Use database")
            _exec_sql(cur, "USE SCHEMA AGENTIC_DOC_EXTRACTION", "Use schema")
...
if __name__ == "__main__":
    if DEPLOY_MODE:
        ...
        print(
            "▶️  To run: prefect deployment run 'S3 to Snowflake Document Extraction/s3-to-snowflake-extraction'"
        )
